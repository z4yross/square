...
public class Labyrinth extends Environment{
  ...
  protected Array<Agent> failAgents = new Array<Agent>();
...
//            System.out.println("Die");
            ...
            break;
        case 2: // advance
            if (!((Boolean) p.get("front")).
                booleanValue() &&
                ( p.get("afront") == null || !((Boolean) p.get("afront")).
                booleanValue() ) ) {
                switch (direction) {
                case 0:
                    y--;
  ...
  public Labyrinth( Array<Agent> _agents, int[][] _structure ) {
    super( _agents );
    for( Agent a:agents ){
  ...
  public Array<Action> actions(){
    Array<Action> acts = new Array<Action>();
  ...
  public void setAgentPosition( int agent_id, int x, int y, int d ){
    SimulatedAgent agent = (SimulatedAgent)agent(agent_id);
