...
public class MultiAgentLabyrinth extends Labyrinth {
  ...
  public Percept sense(Agent agent){
	...
	p.set(LabyrinthUtil.AGENT[i], false);
    }
    ...
    for( Agent ag:agents ){
        if( ag != agent ){
            ...
            for( int k=0; k<LabyrinthUtil.AGENT.length; k++ ){
        		if( !((Boolean)p.get(LabyrinthUtil.WALL[k])).booleanValue() && 
                      pos[1][k] == ay && pos[0][k]==ax  )
        		    p.set(LabyrinthUtil.AGENT[k], true);
            }
        }    
    }
    ...
    p.set(LabyrinthUtil.FAIL, i<failAgents.size());
    return p;
  }

  public MultiAgentLabyrinth( Array<Agent> _agents, int[][] _structure ) {
