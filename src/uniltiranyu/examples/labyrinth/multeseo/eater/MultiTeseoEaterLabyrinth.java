...
import uniltiranyu.examples.labyrinth.multeseo.MultiAgentLabyrinth;
...
public class MultiTeseoEaterLabyrinth extends MultiAgentLabyrinth{
    ...
    public MultiTeseoEaterLabyrinth(Array<Agent> _agents, int[][] _structure) {
	...
	int i=agentIndex(agent);
	p.set(LabyrinthUtil.ENERGY, agent_energy_level[i]);
	return p;
    }   
	    ...
	    if( flag ){
	      ...
	      switch (actionID) {
	        ...
	        default:
	          ...
	          if( actionID != 2 || !((Boolean) p.get("afront")).booleanValue() ) super.act(agent, action);
	          else {
	                msg = SimpleView.ERROR +
